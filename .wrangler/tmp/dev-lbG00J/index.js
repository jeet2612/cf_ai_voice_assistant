...
var src_default = {
  async fetch(request, env2) {
    ...
    if (url.pathname === "/chat" && request.method === "POST") {
      ...
      const id = env2.CONVERSATION.idFromName(sessionId || "default");
      const stub = env2.CONVERSATION.get(id);
      ...
      const messages = [
        {
          role: "system",
          content: "You are a helpful, friendly voice assistant. Keep responses concise and conversational \u2014 ideally 2-3 sentences. Avoid markdown, bullet points, or special characters since your responses will be read aloud."
        },
        ...history
      ];
      const aiResponse = await env2.AI.run("@cf/meta/llama-3.3-70b-instruct-fp8-fast", {
        messages,
        max_tokens: 300
      });
    ...
    if (url.pathname === "/history" && request.method === "GET") {
      ...
      const id = env2.CONVERSATION.idFromName(sessionId);
      const stub = env2.CONVERSATION.get(id);
...
// .wrangler/tmp/bundle-3LWybu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3LWybu/middleware-loader.entry.ts
